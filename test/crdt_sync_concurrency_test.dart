...
void main() {
  group('Concurrent Operations and Edge Cases', () {
    test('simultaneous writes from both client and server', () async {
      ...
      final expectedRecords = 20; // 10 from each side
      ...
      await allSynced.future.timeout(Duration(seconds: 5));
    ...
    test('conflict resolution with same key updates', () async {
      ...
      await conflictResolved.future.timeout(Duration(seconds: 3));
      ...
      // Both sides should have the same final version (CRDT ensures convergence)
    ...
    test('high-frequency updates with batching', () async {
      ...
      await batchingComplete.future.timeout(Duration(seconds: 5));

      // Verify batching occurred (should receive fewer changesets than individual writes)
    ...
    test('large changeset handling', () async {
      ...
      final bulkData = <Future>[];
      for (int i = 0; i < 1000; i++) {
        bulkData.add(client.put('bulk_data', 'item_$i', {
          'id': i,
          'name': 'Item $i',
          'description':
              'This is a description for item $i with some additional text to make it larger.',
      ...
      await bulkSyncComplete.future.timeout(Duration(seconds: 10));
    ...
    test('connection state consistency during rapid connect/disconnect',
      ...
      await allConnected.future.timeout(Duration(seconds: 3));
      ...
      await allDisconnected.future.timeout(Duration(seconds: 3));
